...
## 🔐 Generate SSH Key with `tls_private_key`
Choose **one** algorithm:

### Option A — ED25519 (modern, fast)
```hcl
...
- `private_key_pem` — PEM‑encoded private key (**sensitive; ends up in state**)
- `public_key_openssh` — OpenSSH formatted public key (safe to upload)

---

## 🗃️ Save Private Key to Disk (secure permissions)
```hcl
...
## ☁️ Create AWS EC2 Key Pair from the Public Key
```hcl
...
## 🌐 Minimal Networking (Default VPC)
```hcl
provider "aws" {
  region = "us-east-1"
}
...
## 💻 Launch EC2 with the TLS‑Generated Key
```hcl
...
## 🧰 Optional — Store Public Key as a Separate File
...
## ⚠️ Security Caveats & Best Practices
...
- Prefer generating keys **outside Terraform** for long‑lived production access, and only reference the public key (`aws_key_pair.public_key`).
...
├── main.tf               # tls_private_key + local_file + aws_key_pair + ec2
